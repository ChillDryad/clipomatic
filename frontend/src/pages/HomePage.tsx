  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-card p-5">
        <h1 className="text-2xl font-bold text-[var(--ctp-text)] mb-2">Start New Project</h1>
        <p className="text-[var(--ctp-subtext)] mb-5">Upload a video or paste a YouTube/Twitch URL to begin</p>
        ...
        <div className="flex gap-3 flex-col sm:flex-row">
          ...
          />
          <Link
            to={urlInput ? `/pipeline?url=${encodeURIComponent(urlInput)}` : '/pipeline'}
            className="btn-primary px-6 py-3 rounded-lg whitespace-nowrap min-h-[44px] flex items-center justify-center"
          >
            Start Pipeline
          </Link>
        </div>
      ...
      <section aria-labelledby="recent-heading">
        <h2 id="recent-heading" className="text-lg font-semibold text-[var(--ctp-text)] mb-3">Recent Videos</h2>

        {loading ? (
          <div className="glass-card p-6 text-center" role="status" aria-live="polite">
            <span className="text-[var(--ctp-subtext)]">Loading...</span>
          </div>
        ) : loadError ? (
          <div className="glass-card p-6 text-center" role="alert">
            <p className="text-[var(--ctp-red)] mb-2">{loadError}</p>
            <button
              onClick={() => { setLoadError(null); setLoading(true); getRecentVideos().then(setVideos).catch(e => { setLoadError('Failed to load videos'); setLoading(false) }).finally(() => setLoading(false)) }}
          ...
          <div className="glass-card p-6 text-center">
            <p className="text-[var(--ctp-subtext)]">No videos yet. Start a project above!</p>
          </div>
        ) : (
...
function VideoCard({ video }: { video: RecentVideo }) {
  ...
  const stem = video.sourcePath.split('/').pop()?.replace(/\.[^.]+$/, '') ?? video.title
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-hidden="true"
      >
        <svg className="w-10 h-10 text-[var(--ctp-overlay)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
          <path strokeLinecap="round" strokeLinejoin="round" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" />
        </svg>
      </div>
      <h3 className="font-semibold text-[var(--ctp-text)] truncate">{stem}</h3>
      <p className="text-sm text-[var(--ctp-subtext)]">{video.clipCount} clips</p>
    </Link>
  )
}