...
import {
	loadTimelineDataByProject,
	updateClipMetadata,
	regenerateClipMetadata,
	renderClip,
	createMarker,
	listMarkers,
	updateMarker as updateMarkerApi,
	deleteMarker as deleteMarkerApi,
	uploadMedia,
	listMedia,
	uploadAudio,
	downloadSegment,
	...
	cancelOperation,
	getCachedTranscript,
	type MediaAsset as ApiMediaAsset,
} from "../api";
import { OpenCutTimeline } from "../components/timeline/OpenCutTimeline";
import {
	TimelineToolbar,
	type RenderSettings,
} from "../components/timeline/TimelineToolbar";
...
} from "../components/panels/KeyboardShortcutsModal";
...
export function TimelineEditorPage() {
	...
	const { open: shortcutsOpen, onClose: closeShortcuts } =
		useKeyboardShortcutsHelp();
	...
	const [segmentDownloadProgress, setSegmentDownloadProgress] = useState<{
		p: number;
		l: string;
	} | null>(null);
	...
	const selectedSegmentId =
		selectedSegmentIds.size > 0 ? Array.from(selectedSegmentIds)[0] : null;
	const selectedSegment =
		tracks.flatMap((t) => t.segments).find((s) => s.id === selectedSegmentId) ??
		null;
		...
		const loadTimeline = async () => {
			try {
				...
				try {
					...
					const dimsRes = await fetch(
						`/api/video/dimensions?path=${encodeURIComponent(videoPath)}`,
						{ credentials: "include" },
					);
					videoDimensions = dimsRes.ok
						? await dimsRes.json()
						: { w: 1920, h: 1080 };
				...
				const subtitleTrack = useTimelineStore
					.getState()
				...
				if (sourceUrlToUse) {
					setSegmentDownloadProgress({
						p: 0,
						l: "Downloading clip segment for editing…",
					});
					try {
	...
	const handleUpdateSegment = useCallback(
		(trackId: string, segmentId: string, patch: Partial<TrackSegment>) => {
			pushUndo();
	...
	const handleSplit = useCallback(() => {
		...
		const newSegment: TrackSegment = {
			...
			...(segment.type === "subtitle"
				? { text: segment.text, words: segment.words }
				: {}),
	...
	const handleCropChange = useCallback(
		(avatar: CropBox, gameplay: CropBox) => {
			pushUndo();
	...
	const handleUpdateMarker = useCallback(
		async (markerId: string, patch: Partial<import("../types").Marker>) => {
			try {
	...
	const handleSelectMedia = useCallback(
		(asset: import("../components/panels/MediaLibrary").MediaAsset) => {
			pushUndo();
	...
	const handleRegenerateMetadata = useCallback(async () => {
		...
		try {
			const updated = await regenerateClipMetadata(
				clipId,
				clip,
	...
	const handleSave = async () => {
		...
		try {
			...
			navigate(`/video/${projectId}`, {
				state: { modifiedClip: clip } as EditorNavigationState,
			});
		} catch (err) {
	...
	const executeRender = useCallback(async () => {
		...
		try {
			...
			// Check if videoPath is a segment file (format: <id>_seg_<section_start>.<ext>)
			const isSegmentFile = videoPath.includes("_seg_");
			const segMatch = isSegmentFile
				? videoPath.match(/_seg_([\d.]+)\./)
				: null;
			const segStartTime = segMatch ? parseFloat(segMatch[1]) : 0;

			// Adjust clip timestamps if this is a segment file
			const renderClipData = isSegmentFile
				? {
						...clip,
						start: clip.start - segStartTime,
						end: clip.end - segStartTime,
					}
				: { ...clip };
			...
			const url = await renderClip(
				{
					...
					layout_mode: "stacked",
					animation_speed: renderSettings.animationSpeed,
				},
				(p, l) => setRenderProgress({ p, l }),
				controller.signal,
			);
	...
	if (loading)
		return (
			<div className="text-[var(--ctp-subtext)] p-8">Loading timeline...</div>
		);
	if (error)
		return <div className="text-[var(--ctp-red)] p-8">Error: {error}</div>;
			...
			header={
				<div className="flex items-center gap-2 w-full">
					...
					<div className="flex items-center gap-2">
						{regenerating ? (
							<span className="text-xs text-[var(--ctp-subtext)] animate-pulse">
								Generating…
							</span>
						) : (
							<button
