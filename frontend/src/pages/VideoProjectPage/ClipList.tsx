...
export function ClipList({
	...
	const [cropSegmentPaths, setCropSegmentPaths] = useState<
	...
	const handleRender = async (clip: Clip, clipKey: string) => {
		...
		try {
			...
			// Segment files have format: <id>_seg_<section_start>.<ext>
			const isSegmentFile = sourcePath.includes("_seg_");
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				{clips.map((clip) => {
					...
					return (
						<div
							key={clipKey}
							className="glass-card p-3 col-span-1"
							role="listitem"
						>
